...
func CreateMTGTag(ctx context.Context, input model.MtgCreateTagInput) (*model.MtgTag, error) {
	...
	aq := arango.NewQuery( /* aql */ `
		INSERT { name: @name } INTO mtg_tags
		RETURN { _key: NEW._key, name: NEW.name }
	`)
...
// AssignTagToCard creates an edge from the tag to the card. Idempotent: no-op if edge already exists.
func AssignTagToCard(ctx context.Context, input model.MtgAssignTagToCardInput) (*model.Response, error) {
	...
	aq := arango.NewQuery( /* aql */ `
		...
		LET existing = (FOR e IN mtg_tag_to_card FILTER e._from == fromRef AND e._to == toRef LIMIT 1 RETURN 1)
		FILTER LENGTH(existing) == 0
		INSERT { _from: fromRef, _to: toRef } INTO mtg_tag_to_card
		RETURN NEW
	`)
	...
	if mtgCardSearch.IsIndexReady() {
		if tags, err := GetTagsForCard(ctx, input.CardID); err == nil {
			mtgCardSearch.UpdateCardTagsInIndex(input.CardID, tags)
		}
	}
...
// UnassignTagFromCard removes the edge from the tag to the card.
