...
type Card struct {
	...
	ColorIdentity  []string          `json:"color_identity"`  // This card's color identity.
	ColorIndicator *[]string         `json:"color_indicator"` // The colors in this card's color indicator, if any. A null value for this field indicates the card does not have one.
	Colors         *[]string         `json:"colors"`          // This card's colors, if the overall card has colors defined by the rules. Otherwise the colors will be on the card_faces objects.
	...
	BorderColor      scryfallModel.BorderColor `json:"border_color"`      // This card's border color: black, white, borderless, silver, or gold.
	...
	Games            []scryfallModel.Game      `json:"games"`             // A list of games that this card print is available in paper, arena, and/or mtgo.
	...
	ImageStatus      scryfallModel.ImageStatus `json:"image_status"`      // A computer-readable indicator for the state of this card's image generation. One of missing, placeholder, lowres, or highres_scan.
	...
	Rarity           scryfallModel.Rarity      `json:"rarity"`            // This card's rarity. One of common, uncommon, rare, special, mythic, or bonus.
...
type CardFace struct {
	...
	Object          scryfallModel.CardFaceObject `json:"object"`            // A content type for this object, always card_face.
...
type Set struct {
	...
	Code          string                  `json:"code"`            // The unique three to six-letter code for this set.
	MTGOCode      *string                 `json:"mtgo_code"`       // The unique code for this set on MTGO, which may differ from the regular code.
	ArenaCode     *string                 `json:"arena_code"`      // The unique code for this set on Arena, which may differ from the regular code.
	TCGPlayerID   *int                    `json:"tcgplayer_id"`    // This set's ID on TCGplayer's API, also known as the groupId.
	Name          string                  `json:"name"`            // The English name of the set.
	SetType       scryfallModel.SetType   `json:"set_type"`        // A computer-readable classification for this set.
	ReleasedAt    *string                 `json:"released_at"`     // The date the set was released or the first card was printed in the set (in GMT-8 Pacific time).
	BlockCode     *string                 `json:"block_code"`      // The block code for this set, if any.
	Block         *string                 `json:"block"`           // The block or group name code for this set, if any.
	ParentSetCode *string                 `json:"parent_set_code"` // The set code for the parent set, if any. promo and token sets often have a parent set.
	CardCount     int                     `json:"card_count"`      // The number of cards in this set.
	PrintedSize   *int                    `json:"printed_size"`    // The denominator for the set's printed collector numbers.
	Digital       bool                    `json:"digital"`         // True if this set was only released in a video game.
	FoilOnly      bool                    `json:"foil_only"`       // True if this set contains only foil cards.
	NonFoilOnly   bool                    `json:"nonfoil_only"`    // True if this set contains only nonfoil cards.
	ScryfallURI   string                  `json:"scryfall_uri"`    // A link to this set's permapage on Scryfall's website.
	URI           string                  `json:"uri"`             // A link to this set object on Scryfall's API.
	IconSVGURI    string                  `json:"icon_svg_uri"`    // A URI to an SVG file for this set's icon on Scryfall's CDN.
	SearchURI     string                  `json:"search_uri"`      // A Scryfall API URI that you can request to begin paginating over the cards in this set.
	// ETag is intentionally omitted from the Scryfall API documentation, but present in the original struct. Keeping it.
...
type RelatedCard struct {
	...
	Object    scryfallModel.RelatedCardObject    `json:"object"`    // A content type for this object, always related_card.
